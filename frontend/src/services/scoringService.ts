...
export const scoringService = {
  ...
  async initializeCompetencies(): Promise<{ success: boolean; message?: string }> {
    try {
      const response = await apiClient.post('/api/scoring/initialize-competencies');
  ...
  async calculateScores(sessionId: string, recalculate?: boolean): Promise<ScoringResults> {
    try {
      ...
      const response = await apiClient.get(`/api/scoring/sessions/${sessionId}/calculate-scores`, { params });
  ...
  async generateFinalReport(sessionId: string, userId: string, includeFeedback?: boolean): Promise<FinalReport> {
    try {
      ...
      const response = await apiClient.get(`/api/scoring/sessions/${sessionId}/users/${userId}/final-report`, { params });
  ...
  async exportResults(sessionId: string, format: 'csv' | 'json' = 'csv', includeDetails?: boolean): Promise<Blob> {
    try {
      ...
      const response = await apiClient.get(`/api/scoring/sessions/${sessionId}/export`, {
        params,
        responseType: 'blob'
      });
