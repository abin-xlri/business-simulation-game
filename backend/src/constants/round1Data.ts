...
export const COLD_CHAIN_PENALTY = 100000; // ₹100,000 per breach
export const MAX_SEGMENT_TIME = 2; // hours

// Partner Selection Models for Round 1 Task 2
...
// Script-aligned partner criteria for board guidance
export interface PartnerCriteria {
  networkCoverage: number;       // provinces count normalized to 1-10
  speedOfOnboardingDays: number; // lower is better
  coldChainInfraScore: number;   // 1-10 (higher is better)
  communityTrustScore: number;   // 1-10
  moHCoordinationScore: number;  // 1-10
  dataReportingScore: number;    // 1-10
  operationalRiskScore: number;  // 1-10 (lower is better)
  scalabilityYear2Score: number; // 1-10
  biologicsExperienceScore: number; // 1-10
  costPerPatient: number;        // absolute cost; lower is better
  mediaPoliticalRiskScore: number; // 1-10 (lower is better)
}
...
export const MAX_PARTNER_SCORE = 10;