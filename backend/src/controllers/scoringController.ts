...
import { COMPETENCIES, TASK_COMPETENCY_MAPPINGS, TASK_NAMES, SCORING_THRESHOLDS, FEEDBACK_TEMPLATES } from '../constants/competencies';
...
export class ScoringController {
  ...
  static async initializeCompetencies() {
    try {
      ...
      for (const [taskType, mappings] of Object.entries(TASK_COMPETENCY_MAPPINGS)) {
  ...
  static async calculateScores(req: Request, res: Response) {
    try {
      ...
      const userScores: UserTotalScore[] = [];

      for (const userSession of session.userSessions) {
        const userScore = await this.calculateUserScore(userSession.userId, sessionId);
  ...
  private static async calculateUserScore(userId: string, sessionId: string): Promise<UserTotalScore> {
  ...
  private static async calculateTaskScore(userId: string, sessionId: string, taskType: TaskType): Promise<TaskScore | null> {
    ...
    switch (taskType) {
      case TaskType.ROUND1_TASK1_ROUTE_OPTIMIZATION:
      ...
      case TaskType.ROUND1_TASK2_PARTNER_SELECTION:
      ...
      case TaskType.ROUND3_TASK1_CRISIS_WEB:
      ...
      case TaskType.ROUND3_TASK2_REACTIVATION_CHALLENGE:
      ...
      case TaskType.ROUND2_GROUP_MARKET_SELECTION:
      case TaskType.ROUND2_GROUP_BUDGET_ALLOCATION:
    ...
    if (!submission && taskType !== TaskType.ROUND2_GROUP_MARKET_SELECTION && taskType !== TaskType.ROUND2_GROUP_BUDGET_ALLOCATION) {
      return null;
    }
  ...
  private static async calculateGroupTaskScore(userId: string, sessionId: string, taskType: TaskType) {
  ...
  private static async calculateTaskCompetencyScores(userId: string, sessionId: string, taskType: TaskType): Promise<CompetencyScore[]> {
    const taskMappings = TASK_COMPETENCY_MAPPINGS[taskType] || [];
  ...
  private static async calculateCompetencyScore(userId: string, sessionId: string, taskType: TaskType, competencyId: string) {
    let score = 0;
    let maxScore = 100;
    let evidence: string[] = [];
  ...
  private static async calculateAnalyticalThinkingScore(userId: string, sessionId: string, taskType: TaskType): Promise<number> {
    // Route optimization: Based on profit optimization
    if (taskType === TaskType.ROUND1_TASK1_ROUTE_OPTIMIZATION) {
  ...
  private static async calculateStrategicPlanningScore(userId: string, sessionId: string, taskType: TaskType): Promise<number> {
    // Partner selection: Based on strategic alignment
    if (taskType === TaskType.ROUND1_TASK2_PARTNER_SELECTION) {
  ...
  private static async calculateCollaborationScore(userId: string, sessionId: string, taskType: TaskType): Promise<number> {
    // Group tasks: Based on participation and contribution
    if (taskType === TaskType.ROUND2_GROUP_MARKET_SELECTION || taskType === TaskType.ROUND2_GROUP_BUDGET_ALLOCATION) {
      const userGroups = await prisma.group.findMany({
        where: {
          sessionId,
          members: { some: { userId } }
  ...
  private static async calculateLeadershipScore(userId: string, sessionId: string, taskType: TaskType): Promise<number> {
  ...
  private static async calculateProblemSolvingScore(userId: string, sessionId: string, taskType: TaskType): Promise<number> {
    // Crisis management: Based on effectiveness
    if (taskType === TaskType.ROUND3_TASK1_CRISIS_WEB) {
  ...
  private static async calculateDecisionMakingScore(userId: string, sessionId: string, taskType: TaskType): Promise<number> {
  ...
  private static async calculateCommunicationScore(userId: string, sessionId: string, taskType: TaskType): Promise<number> {
  ...
  private static async calculateAdaptabilityScore(userId: string, sessionId: string, taskType: TaskType): Promise<number> {
    ...
    if (taskType === TaskType.ROUND3_TASK1_CRISIS_WEB) {
      ...
      if (submission) {
        // riskLevel stored as 'Low' | 'Medium' | 'High' -> map to numeric 0..1 and compute inverse
        const map: Record<string, number> = { Low: 0.2, Medium: 0.5, High: 0.8 };
        const numeric = map[submission.riskLevel] ?? 0.5;
  ...
  private static async calculateInnovationScore(userId: string, sessionId: string, taskType: TaskType): Promise<number> {
  ...
  private static async calculateExecutionScore(userId: string, sessionId: string, taskType: TaskType): Promise<number> {
    // Reactivation challenge: Based on implementation effectiveness
    if (taskType === TaskType.ROUND3_TASK2_REACTIVATION_CHALLENGE) {
  ...
  private static async calculateCompetencyScores(userId: string, sessionId: string): Promise<CompetencyScore[]> {
    const competencyScores: CompetencyScore[] = [];
  ...
  private static async calculateTaskBreakdown(sessionId: string, userScores: UserTotalScore[]) {
    const breakdown: any = {};
  ...
  private static async calculateCompetencyBreakdown(userScores: UserTotalScore[]) {
    const breakdown: any = {};

    for (const category of Object.values(CompetencyCategory)) {
  ...
  static async generateFinalReport(req: Request, res: Response) {
    try {
      ...
      const finalReport = await prisma.finalReport.upsert({
        ...
        update: {
          ...
          rank: userScore.rank,
          competencyScores: JSON.stringify(userScore.competencyScores),
          feedback: includeFeedback ? JSON.stringify(await this.generateFeedback(userScore)) : JSON.stringify([]),
  ...
  private static async generateFeedback(userScore: UserTotalScore): Promise<FeedbackItem[]> {
    const feedback: FeedbackItem[] = []; // Changed type to any[]

    for (const competencyScore of userScore.competencyScores) {
      const competency = COMPETENCIES.find(c => c.id === competencyScore.competencyId);
      if (!competency) continue;

      const template = FEEDBACK_TEMPLATES[competency.category];
      const percentage = competencyScore.percentage;

      let strengths: string[] = [];
      let areasForImprovement: string[] = [];

      if (percentage >= SCORING_THRESHOLDS.EXCELLENT) {
        strengths = template.strengths;
        areasForImprovement = ['Continue to develop and refine these skills'];
      } else if (percentage >= SCORING_THRESHOLDS.GOOD) {
        strengths = template.strengths.slice(0, 2);
        areasForImprovement = template.areasForImprovement.slice(0, 1);
      } else if (percentage >= SCORING_THRESHOLDS.SATISFACTORY) {
  ...
  static async exportResults(req: Request, res: Response) {
    try {
      ...
      // Get the session and calculate scores
      const session = await prisma.session.findUnique({
        where: { id: sessionId },
  ...
  // Generate CSV data
  private static generateCSV(results: ScoringResults): string {
    const headers = [
      'Rank',
      'User Name',
      'Total Score',
    ...
    const rows = results.userScores.map((user: UserTotalScore) => [
      user.rank,
      user.userName,
      user.totalScore,
      user.overallPercentage.toFixed(2),
      ...user.competencyScores.map((cs: CompetencyScore) => cs.percentage.toFixed(2))
    ]);
