...
import { CRISIS_SCENARIOS, CRISIS_ADVISORS, CRISIS_ACTIONS, REACTIVATION_NODES, CRISIS_CONSTRAINTS, REACTIVATION_CONSTRAINTS } from '../constants/crisisData';

const prisma = new PrismaClient();

export class CrisisController {
  // Get crisis data for Task 1
  static async getCrisisData(req: Request, res: Response) {
    try {
      res.json({
        scenarios: CRISIS_SCENARIOS,
        advisors: CRISIS_ADVISORS,
        actions: CRISIS_ACTIONS,
        constraints: {
          maxAdvisors: CRISIS_CONSTRAINTS.MAX_ADVISORS,
          maxActions: CRISIS_CONSTRAINTS.MAX_ACTIONS,
          totalPoints: CRISIS_CONSTRAINTS.TOTAL_POINTS
        }
      });
    } catch (error) {
  ...
  static async getReactivationData(req: Request, res: Response) {
    try {
      res.json({
        nodes: REACTIVATION_NODES,
        constraints: {
          maxParallelTasks: REACTIVATION_CONSTRAINTS.MAX_PARALLEL_TASKS,
          criticalPathTime: REACTIVATION_CONSTRAINTS.CRITICAL_PATH_TIME,
          resourceLimits: REACTIVATION_CONSTRAINTS.RESOURCE_LIMITS
        }
      });
    } catch (error) {
  ...
  static async validateCrisisWeb(req: Request, res: Response) {
    try {
      const { selectedAdvisors, selectedActions } = req.body;
      const validation = CrisisController.validateCrisisWebConstraints(
        selectedAdvisors || [],
        selectedActions || []
      );
  ...
  static async submitCrisisWeb(req: Request, res: Response) {
    try {
      ...
      if (a.malaysia && Array.isArray(a.malaysia.actions)) {
        const cost = a.malaysia.actions.reduce((sum: number, id: string) => {
          const act = CRISIS_ACTIONS.find(x => x.id === id); return sum + (act?.cost || 0);
        }, 0);
        if (cost > 30) {
      ...
      const effectiveness = CrisisController.calculateEffectiveness(
        selectedAdvisors,
        selectedActions
      );
      const riskLevel = CrisisController.calculateRiskLevel(
        selectedAdvisors,
        selectedActions
      );
  ...
  static async validateReactivation(req: Request, res: Response) {
    try {
      const { sequence } = req.body;
      
      const validation = CrisisController.validateReactivationConstraints(
        sequence || []
      );
  ...
  static async submitReactivation(req: Request, res: Response) {
    try {
      const { sequence } = req.body;
      const userId = (req as any).user?.userId;
      const sessionId = req.params.sessionId;
  ...
  private static validateCrisisWebConstraints(
    ...
    // Check advisor limit
    if (selectedAdvisors.length > CRISIS_CONSTRAINTS.MAX_ADVISORS) {
      violations.push({
        type: 'advisor_limit',
        message: `Maximum ${CRISIS_CONSTRAINTS.MAX_ADVISORS} advisors allowed`,
        severity: 'error'
      });
    }

    // Check action limit
    if (selectedActions.length > CRISIS_CONSTRAINTS.MAX_ACTIONS) {
      violations.push({
        type: 'action_limit',
        message: `Maximum ${CRISIS_CONSTRAINTS.MAX_ACTIONS} actions allowed`,
        severity: 'error'
      });
    }
    ...
    selectedAdvisors.forEach(advisorId => {
      const advisor = CRISIS_ADVISORS.find(a => a.id === advisorId);
      if (advisor) {
    ...
    const has = (id: string) => selectedActions.includes(id);
    const advisor = (id: string) => selectedAdvisors.includes(id);
    ...
    // Check point budget
    if (totalCost > CRISIS_CONSTRAINTS.TOTAL_POINTS) {
      violations.push({
        type: 'point_budget',
        message: `Total cost (${totalCost}) exceeds budget (${CRISIS_CONSTRAINTS.TOTAL_POINTS})`,
        severity: 'error'
      });
    }

    const remainingPoints = CRISIS_CONSTRAINTS.TOTAL_POINTS - totalCost;
  ...
  private static validateReactivationConstraints(
    ...
    const violations: ConstraintViolation[] = [];
    const resourceConflicts: Record<string, number> = {};
    ...
    const indexOf = (id: string) => sequence.indexOf(id);
    const mustPrecede = (before: string, after: string) => indexOf(before) !== -1 && indexOf(after) !== -1 && indexOf(before) < indexOf(after);

    // E must precede B; E and B must precede A; E and B must precede C; A and C must precede D
    if (!(mustPrecede('E', 'B'))) violations.push({ type: 'dependency_violation', message: 'B requires E1 first', severity: 'error' });
    if (!(mustPrecede('E', 'A') && mustPrecede('B', 'A'))) violations.push({ type: 'dependency_violation', message: 'A requires E1 and B1 first', severity: 'error' });
    if (!(mustPrecede('E', 'C') && mustPrecede('B', 'C'))) violations.push({ type: 'dependency_violation', message: 'C requires B1 and E1 first', severity: 'error' });
    if (!(mustPrecede('A', 'D') && mustPrecede('C', 'D'))) violations.push({ type: 'dependency_violation', message: 'D requires A1 and C1 first', severity: 'error' });

    // Compute total duration with max 2 concurrent nodes. Each node = 4 days when fully restored.
    const totalNodes = sequence.length;
    const totalDays = Math.ceil((totalNodes * 4) / REACTIVATION_CONSTRAINTS.MAX_PARALLEL_TASKS);
    const totalDuration = totalDays * 24; // hours
    const criticalPathTime = totalDuration / REACTIVATION_CONSTRAINTS.MAX_PARALLEL_TASKS;
  ...
  private static calculateEffectiveness(
    ...
    selectedActions.forEach(aid => {
      const a = CRISIS_ACTIONS.find(x => x.id === aid);
      if (a) total += a.effectiveness;
    });

    // Advisor synergies: apply small bonuses
    if (selectedAdvisors.includes('surya_nair') && selectedActions.includes('A1')) total += 1.5;
    if (selectedAdvisors.includes('alisha_tan') && selectedActions.includes('A4')) total += 1.5;
    if (selectedAdvisors.includes('ritika_sen') && selectedActions.includes('A2')) total += 0.5;
    if (selectedAdvisors.includes('deep_mehta') && selectedActions.includes('A3')) total += 1.0;
  ...
  private static calculateRiskLevel(
    ...
    selectedActions.forEach(id => {
      const a = CRISIS_ACTIONS.find(x => x.id === id);
      if (!a) return;
      riskScore += a.risk === 'Low' ? 1 : a.risk === 'Medium' ? 2 : 3;
    });
    // Synergy-based mitigation
    if (selectedAdvisors.includes('surya_nair') && selectedActions.includes('A1')) riskScore -= 1;
    if (selectedAdvisors.includes('alisha_tan') && selectedActions.includes('A4')) riskScore -= 1;
    if (selectedAdvisors.includes('ritika_sen') && selectedActions.includes('A2')) riskScore -= 0.5;

    return riskScore <= 3 ? 'Low' : riskScore <= 6 ? 'Medium' : 'High';
  }

  private static calculateRiskScore(sequence: string[]): number {
    ...
    const base: Record<string, number> = { A: 3.2, B: 5.4, C: 4.8, D: 6.0, E: 2.6 };
    let remaining = 0;
