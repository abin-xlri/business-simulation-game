...
class AdminController {
    ...
    static async exportResults(req, res) {
        try {
            const { sessionId } = req.params;
            const scores = await AdminController.calculateScores(req, res);
            ...
            const csvRows = scores.map((user) => [
                user.userId,
                user.userName,
                user.totalScore,
                user.taskScores['Route Calculation']?.score || 0,
                user.taskScores['Partner Selection']?.score || 0,
                user.taskScores['Crisis Web']?.score || 0,
                user.taskScores['Reactivation Challenge']?.score || 0
            ]);
