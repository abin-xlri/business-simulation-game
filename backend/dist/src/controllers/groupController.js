...
exports.submitBudgetOrdering = exports.submitMarketDecision = exports.getBudgetData = exports.getMarketData = exports.getMessages = exports.sendMessage = exports.getGroups = exports.getGroup = exports.joinGroup = exports.createGroup = void 0;
...
const getBudgetData = async (req, res) => {
    try {
        res.json({
            ...
            regionGuidance: groupData_1.REGION_GUIDANCE
        });
    }
    catch (error) {
...
const submitMarketDecision = async (req, res) => {
    try {
        const { groupId, rankings, sessionId } = req.body;
        const userId = req.user?.userId;
        if (!userId)
        ...
        const validIds = new Set(groupData_1.MARKET_COUNTRIES.map(c => c.id));
        ...
        if (!resolvedGroupId) {
            ...
            if (!sid)
                return res.status(400).json({ error: 'No active session found for user' });
...
exports.submitMarketDecision = submitMarketDecision;
// Submit Budget Allocation ordering (A–F) for a chosen region (A..F)
const submitBudgetOrdering = async (req, res) => {
    try {
        const { groupId, regionId, order, sessionId } = req.body;
        const userId = req.user?.userId;
        if (!userId)
            return res.status(401).json({ error: 'Authentication required' });
        // Validate region
        if (!groupData_1.REGION_GUIDANCE[regionId]) {
            return res.status(400).json({ error: 'Invalid region id' });
        }
        // Validate ordering
        const validFunctions = new Set(groupData_1.BUDGET_FUNCTIONS.map(f => f.id));
...
exports.submitBudgetOrdering = submitBudgetOrdering;
