...
class SessionOrchestrator {
    constructor() {
        ...
        this.softState = new Map();
    }
    async startSimulation(sessionId) {
        ...
        try {
            if (task === 'ROUND2_MARKET_SELECTION') {
                ...
                if (existing === 0) {
                    ...
                    for (let i = 0; i < userSessions.length; i += chunkSize) {
                        ...
                        await prisma.group.create({
                            data: {
                                ...
                                taskType: client_1.GroupTaskType.MARKET_SELECTION,
                                members: {
                    ...
                    await prisma.group.updateMany({ where: { sessionId }, data: { taskType: client_1.GroupTaskType.MARKET_SELECTION } });
                }
            }
            else if (task === 'ROUND2_BUDGET_ALLOCATION') {
                await prisma.group.updateMany({ where: { sessionId }, data: { taskType: client_1.GroupTaskType.BUDGET_ALLOCATION } });
            }
        }
        catch (err) {
            console.warn('Group orchestration step failed:', err.message);
        }
...
exports.sessionOrchestrator = new SessionOrchestrator();
//# sourceMappingURL=sessionOrchestrator.js.map